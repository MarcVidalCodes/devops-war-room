...
class TriageAgent:
    ...
    def __init__(self, prometheus_url: str = "http://localhost:9090", timeout: int = 10):
        """
        ...
        self.investigation_queries = {
            "HighErrorRate": [
                ...
                ("error_percentage", '(rate(http_errors_5xx_total[5m]) / rate(http_requests_total[5m])) * 100'),
            ],
            "SlowRequests": [
            ...
            "DatabasePoolExhaustion": [
                ...
                ("checkout_request_rate", 'rate(http_requests_total{endpoint="api.checkout"}[5m])'),
            ],
            "MemoryLeak": [
                ("current_leaked_objects", 'memory_leak_objects'),
                ("leak_growth_rate", 'rate(memory_leak_objects[10m])'),
                ("cart_request_rate", 'rate(http_requests_total{endpoint="api.add_to_cart"}[5m])'),
            ],
            "InventoryRaceCondition": [
                ("race_condition_rate", 'rate(inventory_race_conditions_total[5m])'),
                ("inventory_update_rate", 'rate(http_requests_total{endpoint="api.update_inventory"}[5m])'),
                ("race_condition_percentage", '(rate(inventory_race_conditions_total[5m]) / rate(http_requests_total{endpoint="api.update_inventory"}[5m])) * 100'),
            ],
        }
    ...
    def _query_current_metrics(self, alert_name: str) -> Dict[str, Any]:
        ...
        if alert_name not in self.investigation_queries:
            self.logger.warning(f"No investigation queries defined for alert: {alert_name}")
            return metrics
    ...
    def _query_time_series(self, alert_name: str, duration_minutes: int = 15) -> Dict[str, Any]:
        """
    ...
    def _generate_summary(self, alert_name: str, metrics: Dict[str, Any]) -> str:
        ...
        for metric_name, metric_data in metrics.items():
            if isinstance(metric_data, dict) and "error" in metric_data:
                summary_parts.append(f"  - {metric_name}: Error - {metric_data['error']}")
            elif isinstance(metric_data, dict) and "value" in metric_data:
                summary_parts.append(f"  - {metric_name}: {metric_data['value']:.4f}")
            elif isinstance(metric_data, list):
                summary_parts.append(f"  - {metric_name}: {len(metric_data)} data points")
        
        return "\n".join(summary_parts)
    
    def investigate_multiple_alerts(self, alerts: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        ...
        for alert in alerts:
            try:
                ...
                self.logger.error(f"Error investigating alert {alert.get('labels', {}).get('alertname', 'unknown')}: {e}")
                reports.append({
                    "alert_info": {"name": alert.get("labels", {}).get("alertname", "unknown")},
                    "error": str(e),
                })
