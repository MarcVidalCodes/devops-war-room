...
from src.app.bugs import trigger_random_error, memory_leak_cart_session, cause_race_condition, leaked_sessions
import time
...
def get_inventory():
    try:
        trigger_random_error()
        inventory = [{"id": p['id'], "name": p['name'], "stock": p['stock']} for p in products_db]
